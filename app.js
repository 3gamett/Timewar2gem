const STORE = { heroes: 'tw.uni.heroes.v10', skills: 'tw.uni.skills.v10', teams: 'tw.uni.teams.v10' };
...
// --- Unitクラス (getCurrentStatメソッドを確実に定義) ---
class Unit {
...
class BattleEngine {
    constructor(teams) {
        ...
        // createUnitを通してインスタンスを生成することを保証
        this.units = [
    ...
    nextChunk() {
        ...
        if (this.phase === 'opening') {
            ...
            // b.getCurrentStatのエラーを防ぐためにUnitインスタンスを確認
            ...
            if (this.turnIdx < this.turnOrder.length) {
                ...
                if (actor.isAlive()) this.log(`[行動] <b>${actor.name}</b> (通常攻撃)`);
            } else {
                this.turn++; this.viewTurn = this.turn; this.turnIdx = 0;
                if (this.turn > 8) { this.finished = true; this.log("=== 8ターン経過終了 ==="); }
